...
public class UsuarioDAOimpl extends ConexionDB implements UsuarioDAO {
    ...
    public List<Usuario> getAll() throws Exception {
        ...
        try {
            ...
            sql += "UNION ";
            sql += "SELECT U.*, R.rol AS rol, CONCAT_WS(' ', P.nombre, P.ap_paterno, P.ap_materno) AS nombre ";
            sql += "FROM USUARIOS U LEFT JOIN ROLES R ON U.id_rol = R.id LEFT JOIN PACIENTES P ON U.id = P.id WHERE nombre IS NOT NULL ";
            sql += "ORDER BY id ASC";
            this.Conectar();
            PreparedStatement ps = this.conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
